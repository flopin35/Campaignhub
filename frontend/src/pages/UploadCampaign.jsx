...
import { isFreePackage } from '../data/packages';
...
export default function UploadCampaign() {
      ...
      if (isFreePackage(pkg)) {
        try {
          await createNotification({
            userId: user.uid,
            message: 'Free campaign submitted for admin review.',
        ...
        toast('Campaign submitted! Awaiting admin approval.', 'success');
        setDone({ ...created, free: true });
        return;
      }
      ...
      <div className="max-w-2xl mx-auto px-4 py-16">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">
            {isFree ? 'Campaign Submitted' : 'Payment Pending Review'}
          </h2>
          {!isFree && (
            <p className="text-gray-400 mb-2">
              Reference: <strong className="text-brand-400">{done.paymentReference}</strong>
            </p>
          )}
          <p className="text-sm text-gray-500 mb-6">
            {isFree
              ? 'Your free campaign is in the review queue. You will get a share link and QR code once approved.'
              : 'Your campaign goes live after admin verifies payment.'}
          </p>
          <div className="bg-surface-elevated rounded-xl p-4 mb-6">
            <p className="text-xs text-gray-500 mb-1">{isFree ? 'Future public link' : 'Future public link'}</p>
  ...
  const activeSteps = isFreePackage(selectedPkg) ? ['Details', 'Package'] : STEPS;

  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">Launch Campaign</h1>
        <p className="text-gray-500 text-sm">Free to start · upgrade when you need more visibility</p>
        <div className="flex gap-2 mt-4">
          {(step <= 1 ? activeSteps : STEPS).map((s, i) => (
            <div key={s} className={`flex-1 h-1 rounded-full ${i <= step ? 'bg-brand-500' : 'bg-surface-border'}`} />
          ))}
        </div>
        <p className="text-gray-500 text-sm mt-2">
          Step {step + 1} of {step <= 1 ? activeSteps.length : STEPS.length}: {STEPS[step]}
        </p>
      </motion.div>
      ...
      {step === 1 && (
        <div className="space-y-6">
          <h2 className="text-lg font-semibold text-white">Select Your Plan</h2>
          {!draft?.bannerFile && (
          ...
          <button type="button" onClick={() => setStep(0)} disabled={loading} className="btn-secondary w-full disabled:opacity-50">
            ← Back
          </button>
        </div>
      )}
      ...
      {step === 2 && payment && selectedPkg && (
        <div className="space-y-6">
          <PaymentScreen pkg={selectedPkg} paymentReference={payment.paymentReference} onContinue={() => setStep(3)} />
        </div>
      )}
